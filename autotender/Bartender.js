...
// -------------------------- Bartender Domain Specific Business Logic --------------------------
    ...
    var whichCityPrompt = "Which city would you like tide information for?",
        speechOutput = {
			...
			.Welcome to Tide Pooler. "
                + "<audio src='https://s3.amazonaws.com/ask-storage/Bartender/OceanWaves.mp3'/>"
                + whichCityPrompt
                + "</speak>",
...
function getFinalTideResponse(cityStation, date, response) {
    ...
    makeTideRequest(cityStation.station, date, function tideResponseCallback(err, highTideResponse) {
        ...
        response.tellWithCard(speechOutput, "Bartender", speechOutput)
    });
}
